...
import (
	...
	"log";
	"time";
	"github.com/natnael-eyuel-dev/Enhanced-Task-Manager-REST-API/models";
...
func (taskmang *MongoDBTaskManager) collectionRef() *mongo.Collection {
	return taskmang.client.Database(taskmang.database).Collection(taskmang.collection)
}

func (taskmang *MongoDBTaskManager) CreateTask(task *models.Task) (*models.Task, error) {
	...
	collection := taskmang.collectionRef()
...
func (taskmang *MongoDBTaskManager) DeleteTask(taskID string) error {
...
func (taskmang *MongoDBTaskManager) GetAllTasks() ([]models.Task, error) {
...
func (taskmang *MongoDBTaskManager) GetTaskByID(taskID string) (*models.Task, error) {
...
func (taskmang *MongoDBTaskManager) UpdateTask(taskID string, taskUpdate *models.Task) (*models.Task, error) {
...
func (taskmang *MongoDBTaskManager) Close() error {
	contx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	return taskmang.client.Disconnect(contx)
}